...
?>        
    ...
    <body><div id="wait"><div class="loader"></div></div><div id="mainpage">
        <header><div id="sidenavshow" onclick="openNav()">&#9776; </div>
            <img class="hlogo" src="./img/UNICEF_logo_white.png"/><div class="page-title">Outcomes</div>
...
$q->Table='fwoutcomes';
$q->SQL=sprintf('select p.Description "#gProgram", c.* from fwoutcomes c '
        . 'join fwprograms p on p.YOB=c.YOB and p.ProgramId=c.ProgramId where c.YOB=%s and p.SectorId in (%s) order by c.YOB, c.ProgramId',$_SESSION['YOB'], join(',',$sectors));  
$q->Open();
$r=new HithReport($q);
//$r=new Table ($q);
