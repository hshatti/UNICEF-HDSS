...
$q->Table= 'fwreportsector'; //sprintf(sqlBrowseActivities, $_SESSION['username'],$_SESSION['authflag']); 
//alert($q->SQL);
$q->SQL=sprintf('select s.Description Sector,rs.SectorId,rs.YOB,rs.ReportName,rs.CountryId from fwreportsector rs left join fwsectors s on s.SectorId=rs.SectorId where (rs.CountryId=%s or %s=31)',QuotedStr($_SESSION['CountryId']),$_SESSION['authflag']);
$q->Open();
$r=new HithReport($q);
//$r=new Table($q);
